...
class Adv extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('AdvLink', 'required'),
			array('AdvStatus', 'numerical', 'integerOnly'=>true),
			array('AdvTitle, AdvLink', 'length', 'max'=>255),
			array('AdvDiscription, AdvAddDate', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('AdvID, AdvTitle, AdvDiscription, AdvAddDate, AdvStatus, AdvLink', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('AdvLink',$this->AdvLink,true);
